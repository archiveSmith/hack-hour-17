...
function getPINs(observed) {
  ...
  function possibles(char, arr) {
    const adjArr = adjacents[char].concat([char]);
    if (arr.length === 0) return arr.concat(adjArr);
    const result = arr.map((poss) => {
      return adjArr.map(newComb => {
        return newComb + poss;
      });
    });
  ...
  const numStr = observed.toString();
  for (let i = 0, ilen = numStr.length; i < ilen; i += 1) {
      possArr = possibles(numStr[i], possArr);
  }
  return possArr;
}
