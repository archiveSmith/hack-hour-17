...
function getPINs(observed) {
  ...
  let expectations = [], oldExpectations = [];
  for (let i = 0; i < input.length; i += 1) {
    if (i === 0) {
      expectations.push(input[i]);
      adjacents[input[i]].forEach(e => expectations.push(e));
    }
    else {
      oldExpectations = expectations.slice();
      expectations = [];
      for (let j = 0; j < oldExpectations.length; j += 1) {
        expectations.push(oldExpectations[j] + input[i]);
        adjacents[input[i]].forEach(e => expectations.push(oldExpectations[j] + e));
      }
    }
  }
  return expectations;
